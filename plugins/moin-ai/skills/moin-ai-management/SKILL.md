...
- **Hub login** (OAuth, no header) — the supported way, and what the Claude plugin uses. The grant can cover several bots. With one bot in the grant nothing changes; with several, **every tool call must carry a `botId`** argument, and `bot_list` shows which ids the connection covers. Omitting it answers with an error naming the available bots.
- **API key** (`x-api-key` header) — **deprecated**, still accepted for existing connections. One key belongs to one bot, and that is the bot, always. No bot selection anywhere. If a session runs on a key, it keeps working; mention once that the key path is being retired in favour of the Hub login, and point at https://github.com/knowhereto/moin-mcp for the switch (removing the header is all it takes). Do not nag about it again in the same session.
...
- **No moinAI tools at all.** The server is not connected. On Claude Code the fastest fix is the plugin (`claude plugin marketplace add knowhereto/moin-mcp`, then `claude plugin install moin-ai@moin-ai`), which brings the server and this playbook in one go. Otherwise the endpoint is `https://api.moin.ai/mcp` (Streamable HTTP), added **without** an authentication header and authenticated by logging in to the Hub through the OAuth flow; per-client setup instructions live at https://github.com/knowhereto/moin-mcp. Never propose setting up a *new* connection with an `x-api-key` header — that path is deprecated. Point the user at the section for their client rather than guessing at their config file.
- **403 "MCP access is not enabled for this API key".** Only reachable on a (deprecated) key connection: the key is valid but MCP is not switched on for it. In the Hub: bot settings → API settings → enable "Allow MCP access". The key itself does not need to be replaced or re-added. Re-adding the server without the header and logging in to the Hub instead fixes it too, and is the direction to move in anyway.
- **The client says "needs authentication", or offers a login flow.** Which of two opposite things this is depends on how the server was added. **Added without a header** (including the plugin) — this is the real OAuth flow, the normal case: have the user complete it, log in to the Hub and pick bots and scope on the consent page. **Added with a deprecated `x-api-key` header** — the flow leads nowhere: the client is rendering the 403 above as a generic authentication failure and offering a sign-in that does not apply to a key connection. Do not send the user through that one; have them check the MCP toggle instead. If they want to see the real message, a direct request to the endpoint with the key returns it verbatim.
...
- If a tool returns 403 "MCP access is not enabled", the connection runs on a deprecated API key that lacks the MCP toggle in the Hub under API settings — see "When the tools are missing" above.
- If only read tools (list/get/search, playground) appear in the tool list, the connection is scoped **read-only**: on a Hub login the grant covers `mcp:read` only — reconnecting and granting `mcp:write` on the consent page lifts it; on a deprecated API key the key itself needs the "Read & write" access level (Hub → API settings).
- **A write tool that answers "… is not a demo bot" is a permission boundary, not a bug.** A connection authenticated with a moinAI Hub *admin* login may only change bots whose stage is `demo` — every customer bot is read-only for it, while the write tools stay listed either way. `bot_get` reports the stage, `bot_list` marks the writable bots with `demo: true`. Report the boundary and carry on read-only; never retry the call, and never offer to change the bot's stage yourself.
